...
class DataPreProcessing:
    ...
    def count_of_img_for_every_age(self):
        ...
        self.images_df = pd.DataFrame(utk_face_arr.values(), index=utk_face_arr.keys(), columns=['combined_images'])
    ...
    def split_classes(self, n_classes): #used for defining class_labels in datagen.py
        ...
        for i in range(n_classes):
            ...
            classes_df.loc[i, 'Class balance (%)'] = round((class_images / sum(series)) * 100, 2)
    ...
    def generate_df(self):
        ...
        master_df['gender'] = master_df['filename'].map(lambda img_name : np.uint8(img_name.split("_")[1]))
        master_df['target'] = master_df['age'].map(self.class_labels)
        master_df['filename'] = master_df['filename'].map(lambda img_name: os.path.join(self.utk_face_path, img_name))
